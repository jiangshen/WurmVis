...
function recolorHeatmap() {
    /* Removes the old heatmap */
    d3.select("#heatmap").remove().exit();
    /* Removes the old legend */
    d3.select("#gradient-fill").remove().exit();

     /**
     * Color Domain in points per square pixel
     * Mapping extent from all the discrete contours generated by D3 Contour Density
     */
    var colorExtent = d3.extent(contours.map(function(i) { return i.value; }));
    color.domain(colorExtent);
    gradientScale = d3.scaleLinear().domain([0, 1]).range(colorExtent);
    ...
    createGradient();
    d3.select("#gradient-fill-container")
        .append("rect")
